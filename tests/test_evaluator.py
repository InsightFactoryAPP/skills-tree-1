...
class TestMetricCalculations(unittest.TestCase):
    ...
    def _rq(self, recommended, expected_top5):
        """Position-sensitive normalized DCG-style ranking quality."""
        if not recommended or not expected_top5:
            return 0.0
        expected_rank = {skill: i for i, skill in enumerate(expected_top5)}
        score = 0.0
        for i, skill in enumerate(recommended[:5]):
            if skill in expected_rank:
                ideal_gain = 1.0 / (expected_rank[skill] + 1)
                actual_discount = 1.0 / (i + 1)
                score += ideal_gain * actual_discount
        ideal_score = sum(1.0 / (i + 1) ** 2 for i in range(min(5, len(expected_top5))))
        return round(score / ideal_score, 4) if ideal_score > 0 else 0.0
